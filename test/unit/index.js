...
describe("koa-body", () => {
    ...
    router = Router()
      .get("/users", (ctx, next) => {
        if (ctx.request.body && ctx.request.body.name) {
          ctx.body = database.users.find(
            (element) => element.name === ctx.request.body.name
          );
          ctx.status = 200;
          return next();
        }
  ...
  it("should work with defaults - multipart: false, only `urlencoded` and `json` bodies", (done) => {
    ...
    request(createServer(app.callback()))
      .get("/users")
      .expect(200, database)
      .end((err, res) => {
  ...
  it("should receive multiple fields and files via `multipart` on .body.files object", (done) => {
    ...
    request(createServer(app.callback()))
      ...
      .end((err, res) => {
        ...
        res.body._files.firstField.originalFilename.should.equal(
          "package.json"
        );
        should(statSync(res.body._files.firstField.filepath)).be.ok();
        unlinkSync(res.body._files.firstField.filepath);
        ...
        should(statSync(res.body._files.secondField[0].filepath)).be.ok();
        should(statSync(res.body._files.secondField[1].filepath)).be.ok();
        unlinkSync(res.body._files.secondField[0].filepath);
        unlinkSync(res.body._files.secondField[1].filepath);
        ...
        should(statSync(res.body._files.thirdField[0].filepath)).be.ok();
        unlinkSync(res.body._files.thirdField[0].filepath);
        should(statSync(res.body._files.thirdField[1].filepath)).be.ok();
        unlinkSync(res.body._files.thirdField[1].filepath);
        should(statSync(res.body._files.thirdField[2].filepath)).be.ok();
        unlinkSync(res.body._files.thirdField[2].filepath);
  ...
  it("should recieve `urlencoded` request bodies", (done) => {
    ...
    request(createServer(app.callback()))
      ...
      .end((err, res) => {
        ...
        res.body.user.should.have.properties({
          name: "example",
          followers: "41",
        });
  ...
  describe("includeUnparsed tests", () => {
    ...
    it("should not fail when no request body is provided", (done) => {
      const echoRouterLayer = router.stack.filter(
        (layer) => layer.path === "/echo_body"
      );
      requestSpy = spy(echoRouterLayer[0].stack, "0");
    ...
    it("should recieve `urlencoded` request bodies with the `includeUnparsed` option", (done) => {
      ...
      requestSpy = spy(userRouterLayer[0].stack, "0");
    ...
    it("should receive text as strings with `includeUnparsed` option", (done) => {
      ...
      request(createServer(app.callback()))
        ...
        .end((err, res) => {
          ...
          equal(req.body[unparsed], undefined);
  ...
  it("should recieve raw XML with `text` request bodies", (done) => {
    ...
    const requestSpy = spy(echoRouterLayer[0].stack, "0");
  ...
  describe("parsedMethods options", () => {
    ...
    it("cannot use strict mode and parsedMethods options at the same time", (done) => {
      ...
      assert(
        err &&
          err.message ===
            "Cannot use strict and parsedMethods options at the same time."
      );
   ...
   */
  it(
    "should request 413 " + ERR_413_STATUSTEXT + ", because of `formLimit`",
    (done) => {
   ...
   */
  it(
    "should request 413 " + ERR_413_STATUSTEXT + ", because of `jsonLimit`",
    (done) => {
   ...
   */
  it(
    "should request 413 " + ERR_413_STATUSTEXT + ", because of `textLimit`",
    (done) => {
